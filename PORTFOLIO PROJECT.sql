...
('139', '4', '5', '2020-04-10', '9999-12-31'),
('140', '1', '1', '2020-05-10', '9999-12-31'), 
('141', '3', '3', '2020-04-07', '9999-12-31'), ('142', '1', '4', '2020-03-19', '9999-12-31'), ('143', '5', '3', '2020-04-19', '9999-12-31'), ('144', '3', '2', '2020-04-30', '9999-12-31'), ('145', '5', '2', '2020-04-13', '9999-12-31'), ('146', '1', '2', '2020-05-07', '9999-12-31'), ('147', '1', '5', '2020-05-22', '9999-12-31'), ('148', '4', '4', '2020-03-16', '9999-12-31'), ('149', '1', '1', '2020-04-13', '9999-12-31'), ('150', '2', '5', '2020-04-22', '9999-12-31'), ('151', '1', '1', '2020-04-26', '9999-12-31'), ('152', '4', '4', '2020-03-28', '9999-12-31'), ('153', '3', '2', '2020-05-06', '9999-12-31'), ('154', '2', '2', '2020-04-28', '9999-12-31'), ('155', '4', '3', '2020-05-13', '9999-12-31'), ('156', '5', '5', '2020-04-18', '9999-12-31'), ('157', '2', '1', '2020-04-06', '9999-12-31'), ('158', '5', '3', '2020-03-20', '9999-12-31'), ('159', '5', '1', '2020-05-01', '9999-12-31'), ('160', '4', '4', '2020-04-17', '9999-12-31'), ('161', '1', '3', '2020-03-17', '9999-12-31'), ('162', '2', '1', '2020-04-01', '9999-12-31'), ('163', '4', '1', '2020-03-02', '9999-12-31'), ('164', '3', '1', '2020-04-16', '9999-12-31'), ('165', '4', '1', '2020-04-06', '9999-12-31'), ('166', '2', '3', '2020-05-31', '9999-12-31'), ('167', '4', '3', '2020-04-25', '9999-12-31'), ('168', '5', '5', '2020-05-31', '9999-12-31'), ('169', '1', '3', '2020-04-29', '9999-12-31'), ('170', '5', '2', '2020-03-13', '9999-12-31'), ('171', '1', '3', '2020-05-12', '9999-12-31'), ('172', '2', '1', '2020-05-14', '9999-12-31'), ('173', '5', '1', '2020-04-28', '9999-12-31'), ('174', '4', '4', '2020-05-07', '9999-12-31'), ('175', '1', '5', '2020-05-28', '9999-12-31'), ('176', '2', '1', '2020-05-23', '9999-12-31'), ('177', '3', '1', '2020-03-28', '9999-12-31'), ('178', '1', '3', '2020-07-03', '9999-12-31'), ('179', '4', '4', '2020-04-28', '9999-12-31'), ('180', '1', '2', '2020-02-28', '9999-12-31'), ('181', '4', '5', '2020-05-22', '9999-12-31'), ('182', '3', '4', '2020-04-18', '9999-12-31'), ('183', '2', '1', '2020-05-13', '9999-12-31'),
('184', '2', '3', '2020-04-18', '9999-12-31'),
('185', '5', '1', '2020-05-13', '9999-12-31'),
('186', '2', '2', '2020-04-17', '9999-12-31'),
('187', '1', '5', '2020-04-23', '9999-12-31'),
...
select count(distinct customer_id) as unique_codes 
from customer_nodes;
...
select node_id, 
round(AVG((datediff(end_date, start_date))),0) as avg_reallocated_days
from customer_nodes
...
select c.txn_type, count(distinct c.txn_type) as total_count, t.total_amount
from customer_transactions c 
join total t on c.txn_type = t.txn_type
GROUP  by c.txn_type;
...
select customer_id, count(txn_type) as total_count
from customer_transactions
where txn_type = "Deposit"
group by customer_id
)
select AVG(t.total_amount) as avg_total_amount, AVG(c.total_count) as avg_total_count
from total t
join count c on t.customer_id = c.customer_id;

###########--  Q3 For each month- how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?######
select * from customer_transactions;

with monthly as(
select customer_id, MONTHNAME(TXN_DATE) as month, count(customer_id)  as num_customer
from customer_transactions
group by 1,2
order by 1,2
), updated as (
...
count(case when txn_type = "Withdrawal" then customer_id end) as Withdrawal
from customer_transactions
group by customer_id
)
select m.month, count(m.num_customer)
from monthly m
join updated t on t.customer_id = m.customer_id
where (t.deposit_count > 1 and (t.purchase_count or t.withdrawal))
group by m.month;
...
with balance as (
select customer_id, monthname(txn_date) as months,
SUM(txn_amount) over(partition by customer_id order by monthname(txn_date)) as currentclosing
from customer_transactions
), next_balance as (
SELECT customer_id, 
       months,
       LAG(currentclosing, 1) OVER(PARTITION BY customer_id ORDER BY months) AS previousClosing,
       (currentclosing - LAG(currentclosing, 1) OVER(PARTITION BY customer_id ORDER BY months)) / LAG(currentclosing, 1) OVER(PARTITION BY customer_id ORDER BY months) AS percentageChange
FROM balance
), change_in_cus as (
select customer_id
from next_balance 
where percentagechange > 0.05
group by customer_id
) SELECT 
...
##########OPTION 1 Option 1: data is allocated based off the amount of money at the end of the previous month
...
WITH transaction as (
select customer_id, txn_type, month(txn_date) as monthnumber, monthname(txn_date) as monthname,
sum(case when txn_type = "Deposit" then txn_amount else 0 end) as amount
from customer_transactions
where txn_type <> "Purchase" and txn_type <> "Withdrawal"
group by 1,2,3,4
order by 1
), balance as (
select customer_id, monthnumber, monthname,
SUM(amount) OVER(PARTITION BY customer_id, monthnumber ORDER BY customer_id, monthnumber 
ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_balance
from transaction
), avgrunningbal as (
select customer_id, monthnumber, monthname,
AVG(running_balance) as avg_run_bal
from balance
group by 1,2,3
order by 1
)
select monthnumber,monthname,
round(sum(avg_run_bal),2) as total_allocation
from avgrunningbal
group by 1,2
order by 1;
...
#######--3a running customer balance column that includes the impact each transaction##########

select customer_id, month(txn_date), 
sum(case when txn_amount = "Deposit" then txn_amount else -txn_amount end) over (partition by customer_id order by txn_date)  as running_balance
from customer_transactions;


-- 	● customer balance at the end of each month

select customer_id, monthname(txn_date) as months, txn_type,
...
select customer_id, min(running_balance) as minimum, AVG(running_balance) as average, max(running_balance) as maximum
from run_bal
group by customer_id;
...
select  months, round(sum(data_growth),2) as growth_in_data
from (
select customer_id, days, months, allocation, ((allocation*0.06*days)/100) as data_growth
from (
select customer_id, EXTRACT(DAY FROM txn_date) as days, monthname(txn_date) as months, 
sum(case when txn_type = "Deposit" then txn_amount else 0 end) as allocation
from customer_transactions
where txn_type <> "Purchase" and txn_type <> "Withdrawal" 
group by 1,2,3
order by 1
